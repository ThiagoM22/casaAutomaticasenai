...
function Sala() {
  ...
  useEffect(() => {
    ...
    const handleSensorData = (data) => {
      ...
      if (typeof data === "string") {
        ...
        try {
          ...
          console.log(
            "🔍 String não é JSON, tentando extrair valores numericamente..."
          );
          ...
          if (numeros && numeros.length >= 2) {
            ...
            if (tempMatch || humMatch) {
              ...
              console.log(
                "✅ Valores extraídos com regex específica:",
                sensorData
              );
            } else {
              console.warn(
                "⚠️ Não foi possível extrair dados da string:",
                data
              );
              return;
            }
    ...
    const handleACStatus = (status) => {
      console.log(
        "📨 Status AC recebido via MQTT (IGNORADO - usando local):",
        status
      );
    ...
    const handleHumidifierStatus = (status) => {
      console.log(
        "📨 Status umidificador recebido via MQTT (IGNORADO - usando local):",
        status
      );
      // COMENTADO para não interferir com controle local
      // setUmidificador(...)
    ...
    const handleLightStatus = (status) => {
      console.log(
        "📨 Status luz sala recebido via MQTT (IGNORADO - usando local):",
        status
      );
      // COMENTADO para não interferir com controle local
      // setLuzSala(...)
      ...
      <div className={`environment-content ${isExpanded ? "expanded" : ""}`}>
        <div className="sensors">
          <div className="sensor-item">
            ...
            <span className="sensor-source">
              {dataSource === "dht22" ? "DHT22" : "Simulado"}
            </span>
          </div>
          <div className="sensor-item">
        ...
        <div className="controls">
          ...
          <div className="control-item">
            ...
            <span
              className={`status ${
                umidificador === "ligado" ? "status-active" : "status-inactive"
              }`}
            >
              Status: {umidificador}
            </span>
          ...
          <div className="control-item">
            ...
            <span
              className={`status ${
                luzSala === "ligada" ? "status-active" : "status-inactive"
              }`}
            >
              Status: {luzSala}
            </span>
